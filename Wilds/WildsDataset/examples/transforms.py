...
#    t_featurizer = transforms.Lambda(lambda x: featurizer(x))
    ...
    def cutmix(x: torch.Tensor) -> torch.Tensor:
        ...
        # Filtra gli indici in base al cell_type
        cell_type_indices = np.where(dataset._metadata_array[:, 0] == metadata[0])[0]
        ...
        desired_indices = np.intersect1d(train_indices, cell_type_indices)
        desired_indices = np.intersect1d(desired_indices, experiment_indices)
