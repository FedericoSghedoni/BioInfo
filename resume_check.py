...
def main():
    ...
    logs_folders = []
    for log in ['logg', 'log_', 'logc', 'logp_', 'logx', 'logy', 'logw', 'logz', 'logh']: #'logs', 'logg', 'log_', 'logc', 'logp_', 'logx', 'logy', 'logw', 'logz', 'logh'
    ...
    for folder in logs_folders:
        ...
        param_sections = extract_parameters(lines)
        discrepancies = compare_parameters(param_sections)

        if discrepancies:
            print(f"Discrepanze trovate in dir {folder}:")
