...
# makes wilds import from local directory
sys.path.insert(0, 'WildsDataset')
...
def main():
    ...
    if os.path.exists(config.log_dir):
        if config.resume:
            ...
            new_log_dir = config.log_dir
            # Initialize counter for directory names
            counter = 0
            
            # Check if the log directory already exists
            while os.path.exists(new_log_dir):
                # Append counter to the directory name
                counter += 1
                new_log_dir = f'{config.log_dir}_{counter}'
                
            # Rename existing folder to not overwrite it
            os.rename(config.log_dir, new_log_dir)
            resume=False
            mode='w'
    else:
        resume=False
