...
def run_epoch(algorithm, dataset, general_logger, epoch, config, train, unlabeled_dataset=None):
    ...
    for labeled_batch in batches:
        # If ViT model is used adds labels in the labeled_batch dictionary
        if config.model == 'google/vit-base-patch16-224':
            if torch.is_tensor(labeled_batch[0]):
                labeled_batch[0] = {'pixel_values' : labeled_batch[0]}
            labeled_batch[0]['labels'] = labeled_batch[1]
        if train:
            if unlabeled_dataset:
