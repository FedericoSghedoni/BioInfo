...
def initialize_model(config, d_out, is_featurizer=False):
    ...
    elif 'google/vit-base-patch16-224' in config.model:
        if featurize:
            featurizer = initialize_vit_model(config, d_out, featurize)
            classifier = nn.Linear(featurizer.d_out, d_out)
            model = (featurizer, classifier)
        else:
            model = initialize_vit_model(config, d_out)
    ...
    if config.pretrained_model_path is not None:
        ...
        if 'bert' not in config.model and 'vit' not in config.model:  # We've already loaded pretrained weights for bert-based models using the transformers library
            try:
                if featurize:
...
def initialize_vit_model(config, d_out, featurize=False):
    from models.vit import ViTClassifier, ViTFeaturizer

    if config.pretrained_model_path:
        print(f'Initialized model with pretrained weights from {config.pretrained_model_path}')
        config.model_kwargs['state_dict'] = torch.load(config.pretrained_model_path, map_location=config.device)
        config.model_kwargs['state_dict'] = {k[len('model.vit.'):]:v for k,v in config.model_kwargs['state_dict']['algorithm'].items() if k.startswith('model.vit.')}
        
    if config.model == 'google/vit-base-patch16-224':
        train_only_classifier = config.model_kwargs.pop('train_only_classifier', False)
        if featurize:
            model = ViTFeaturizer.from_pretrained(config.model, ignore_mismatched_sizes=True, **config.model_kwargs)
        else:
        ...
        if train_only_classifier:
            # Congela tutti i pesi degli strati convoluzionali
...
def initialize_torchvision_model(name, d_out, **kwargs):
    ...
    train_only_classifier = kwargs.pop('train_only_classifier', False)
