...
class ViTClassifier(ViTForImageClassification):
    ...
    def __call__(self, x):
       ...
       # outputs = super().__call__(**x, output_hidden_states=True) # False se non serve leggere gli embeddings
        ...
        csv_file = 'image_embeddings2.csv'
...
class ViTFeaturizer(ViTModel):
    def __init__(self, config):
        super().__init__(config)
        self.d_out = config.hidden_size  # Dimensione dell'embedding

    def __call__(self, x):
        x["pixel_values"] = x["pixel_values"].squeeze(1)
        x.pop('labels')
        outputs = super().__call__(**x).last_hidden_state
        return outputs[:,0,:]  # Restituisci l'embedding della [CLS] token
